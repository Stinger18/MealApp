...
def create_test_recipe_new_model(db: Session, id: int, ownerId: int, name: str, ingredients: dict, instructions: str, servings: int, prepTime: str, cookTime: str):
    ...
    # Create a new recipe
    newRecipe = models.Recipe(id=id, ownerId=ownerId, name=name, instructions=instructions, servings=servings, prepTime=prepTime, cookTime=cookTime)
    ...
    for ingredient, quantity in ingredients.items(): # For each ingredient in the dictionary
        ...
        models.RecipeIngredient(recipe=newRecipe, ingredient=ingredient, quantity=quantity)
